...
const LeafletMap = forwardRef(function LeafletMap(
  ...
  // ── Edit / Move mode ────────────────────────────────────────────────────────
  // When EDIT mode + activeBoundaryId: replace the static layer with a draggable
  // polygon layer built from individual vertex markers + a live polygon overlay.
  useEffect(() => {
    const map = mapRef.current;
    const Lf  = L.current;
    if (!map || !Lf) return;
    ...
    const ring  = geo.coordinates[0].slice(0, -1); // drop closing duplicate

    // Convert to LatLng
    let latLngs = ring.map(([lng, lat]) => Lf.latLng(lat, lng));

    // Hide static layer while editing
    ...
    function refreshPoly() {
      poly.setLatLngs(markers.map((m) => m.getLatLng()));
    }
    ...
    const markers = latLngs.map((ll, i) => {
      const m = Lf.circleMarker(ll, {
        ...
        .on('mousedown', () => {
          ...
          function onMove(e) { m.setLatLng(e.latlng); refreshPoly(); }
          function onUp() {
            map.dragging.enable();
    ...
    function finishEdit() {
      ...
      const newLatLngs = editLayerRef.current.markers.map((m) => m.getLatLng());
      const coords     = [...newLatLngs, newLatLngs[0]].map((ll) => [ll.lng, ll.lat]);
      const newGeo     = {
        type: 'Feature',
